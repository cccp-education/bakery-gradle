...
@cucumber @bakery
Feature: The generateSite task initialize the static site
  ...
  Scenario: `generateSite` task against empty bakery project
    ...
    Then after running 'generateSite' the task is not available using 'site.yml' configuration
  ...
  # gradle.properties : bakery.config.path='site.yml'
  Scenario: `generateSite` task against empty bakery project using gradle.properties
    Given a new Bakery project
...
#  Scenario: `generateSite` task against an existing bakery project using gradle.properties without site and maquette
#    Given an existing empty Bakery project using DSL with 'site.yml' file
#    And with buildScript file without bakery DSL
#    And does not have 'site.yml' for site configuration
#    And 'settings.gradle.kts' set gradle portal dependencies repository with 'gradlePluginPortal'

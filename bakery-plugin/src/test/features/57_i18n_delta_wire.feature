...
Feature: i18n delta wire — SubtreeI18nPlanner cabled in MigrateContentI18nTask
  ...
  The fixture `cheroliv-com-i18n-deploy` contains 4 French articles covering
  the 3 PlantUml strategies.

  Background:
    Given a cheroliv-com-i18n-deploy fixture with 4 French articles for delta wire
  ...
  Scenario: Fresh migration translates all articles
    ...
    Then the delta-wire task should report "4" files translated for language "en"
  ...
  Scenario: Second run with unchanged source translates nothing
    ...
    And the delta-wire task should report "4" files preserved for language "en"

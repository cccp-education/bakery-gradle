...
class SiteContextCollectorTest {
    ...
    @Test
    fun `should extract articles from feed xml`(
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>http://localhost:8820/blog/2024/second.html</loc></url>
    ...
    @Test
    fun `should return empty articles when no feed xml`(
        @TempDir tempDir: File,
    ) {
        val bakedDir = tempDir.resolve("bake")
        bakedDir.mkdirs()
    ...
    @Test
    fun `should handle empty feed with zero items`(
        @TempDir tempDir: File,
    ) {
        val bakedDir = tempDir.resolve("bake")
        bakedDir.mkdirs()
...
<channel><title>Test</title></channel>
        ...
        fun `augmented entries added when composite context exists`(
            @TempDir tempDir: File,
        ) {
        ...
        fun `no augmented entries when composite context absent`(
            @TempDir tempDir: File,
        ) {
        ...
        fun `no augmented entries when disabled`(
            @TempDir tempDir: File,
        ) {
            ...
            compositeContextFile.writeText(
                """{"eagerSection":"test","ragSection":"","graphifySection":"","docsSection":"","config":{"totalTokenBudget":8000,"budgetEagerLazy":0.4,"budgetRag":0.3,"budgetGraphify":0.2,"budgetDocs":0.1,"budgetOverhead":0.0}}""",
            )
        ...
        fun `maxArticles limits augmented channels`(
            @TempDir tempDir: File,
        ) {
            // GIVEN
            val bakedDir = tempDir.resolve("bake")
        ...
        fun `maxArticles zero means all channels`(
            @TempDir tempDir: File,
        ) {
            // GIVEN
            val bakedDir = tempDir.resolve("bake")

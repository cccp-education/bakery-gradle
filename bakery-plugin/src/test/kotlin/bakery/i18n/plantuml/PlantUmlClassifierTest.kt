 ...
 *
 * Tests unitaires du `PlantUmlClassifier` (domaine pur, no I/O).
 * Classifie les blocs `[plantuml]` extraits des articles `.adoc` en 3
 * stratégies : TranslateLabels (labels traduisibles), PreserveTechnical
 * (diagrammes techniques purs), BorrowVocabulary (vocabulaire métier REAC/AFNOR
 * à préserver).
 *
 * Réutilise l'ontologie de HTOWN EPIC PLT-BOUNDARY (S163-166) —
 * `TranslationStrategy.TRANSLATE/BORROW/PRESERVE` — adaptée au contexte bakery.
 */

...
""")
        ...
        val heading = result.blocks[0] as bakery.pivot.PivotBlock.Heading
        assertEquals("Salut [EN]", heading.text)
        val para = result.blocks[1] as bakery.pivot.PivotBlock.Paragraph
...
""")
        ...
        // inline: Text("Text with "), Code("code"), Text(" and "), Bold("bold"), Text(".")
        val code = para.inline[1] as bakery.pivot.PivotInline.Code
        assertEquals("code", code.text)
        val bold = para.inline[3] as bakery.pivot.PivotInline.Bold
...
""")
        ...
        assertEquals("Introduction [EN]", (result.blocks[0] as bakery.pivot.PivotBlock.Heading).text)
        assertEquals("Sous-section [EN]", (result.blocks[1] as bakery.pivot.PivotBlock.Heading).text)
    }
...
""")
        ...
        val ul = result.blocks[0] as bakery.pivot.PivotBlock.ListBlock
        assertEquals("Item un [EN]", (ul.items[0][0] as bakery.pivot.PivotInline.Text).text)
        assertEquals("Item deux [EN]", (ul.items[1][0] as bakery.pivot.PivotInline.Text).text)
        val ol = result.blocks[1] as bakery.pivot.PivotBlock.ListBlock
        assertEquals("Premier [EN]", (ol.items[0][0] as bakery.pivot.PivotInline.Text).text)
    }
...
""")
        ...
        val table = result.blocks[0] as bakery.pivot.PivotBlock.Table
        assertEquals("Nom [EN]", (table.header[0][0] as bakery.pivot.PivotInline.Text).text)
        assertEquals("Valeur [EN]", (table.header[1][0] as bakery.pivot.PivotInline.Text).text)
    }
...
""")
        ...
        val adm = result.blocks[0] as bakery.pivot.PivotBlock.Admonition
        val para = adm.blocks[0] as bakery.pivot.PivotBlock.Paragraph
        assertTrue(para.inline[0].toString().contains("[EN]"))
    }

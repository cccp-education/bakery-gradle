...
class ScaffoldGeneratorTreeTest {
    ...
    @Test
    fun `generate parses tree when present in LLM response`() =
        runBlocking {
    ...
    @Test
    fun `generate derives templates from tree via flatten when tree present`() =
        runBlocking {
            val fakeLlm = FakeLlmService(sampleTreeJson)
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `generate falls back to templates list when tree absent`() =
        runBlocking {
            val json =
                """
                {
    ...
    @Test
    fun `generate tree present but templates absent uses flatten only`() =
        runBlocking {
            val json =
                """
                {
    ...
    @Test
    fun `generate prefers tree over templates when both present`() =
        runBlocking {
            val json =
                """
                {
    ...
    @Test
    fun `fallback output has null tree`() =
        runBlocking {

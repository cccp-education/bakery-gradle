...
class ScaffoldGeneratorTest {
    ...
    @Test
    fun `generate produces ScaffoldOutput with parsed siteType`() =
        runBlocking {
    ...
    @Test
    fun `generate produces ScaffoldOutput with parsed projectName`() =
        runBlocking {
            val fakeLlm = FakeLlmService(sampleBlogJson)
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `generate produces ScaffoldOutput with parsed description`() =
        runBlocking {
    ...
    @Test
    fun `generate produces ScaffoldOutput with parsed templates`() =
        runBlocking {
            val fakeLlm = FakeLlmService(sampleBlogJson)
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `generate produces ScaffoldOutput with parsed metadata`() =
        runBlocking {
            val fakeLlm = FakeLlmService(sampleBlogJson)
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `generate sends prompt containing the description to LLM`() =
        runBlocking {
            val fakeLlm = FakeLlmService(sampleBlogJson)
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `generate sends prompt containing the site type to LLM`() =
        runBlocking {
            val fakeLlm = FakeLlmService(samplePortfolioJson)
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `generate sends prompt requesting JSON format`() =
        runBlocking {
            val fakeLlm = FakeLlmService(sampleBlogJson)
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `generate handles invalid JSON gracefully`() =
        runBlocking {
    ...
    @Test
    fun `generate handles JSON with missing fields using intention defaults`() =
        runBlocking {
            val partialJson =
                """
                {
    ...
    @Test
    fun `fallback for BLOG type provides blog templates`() =
        runBlocking {
    ...
    @Test
    fun `fallback for DOC type provides documentation templates`() =
        runBlocking {
    ...
    @Test
    fun `fallback for FORMATION type provides formation templates`() =
        runBlocking {
            val fakeLlm = FakeLlmService("")
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `fallback for PORTFOLIO type provides portfolio templates`() =
        runBlocking {
            val fakeLlm = FakeLlmService("")
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `fallback uses description as projectName when projectName is blank`() =
        runBlocking {
            val fakeLlm = FakeLlmService("")
            val generator = ScaffoldGenerator()
    ...
    @Test
    fun `generate produces ScaffoldOutput with tree when LLM returns tree`() =
        runBlocking {
            val json =
                """
                {
    ...
    @Test
    fun `generate derives templates from tree when tree is present`() =
        runBlocking {
            val json =
                """
                {
    ...
    @Test
    fun `generate sets tree to null when LLM response has no tree`() =
        runBlocking {
            val json =
                """
                {
    ...
    @Test
    fun `generate uses tree over legacy templates when both are present`() =
        runBlocking {
            val json =
                """
                {

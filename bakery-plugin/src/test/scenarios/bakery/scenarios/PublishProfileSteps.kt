...
class PublishProfileSteps(
    ...
    fun createFileInSimulatedRemote(
        fileName: String,
        content: String,
    ) {
        val remoteUri =
            world.simulatedRemoteUri
                ?: throw IllegalStateException(
                    "Simulated remote URI not set. Ensure the 'an existing Bakery project with pushProfile pointing to a simulated remote' step was executed first.",
                )
    ...
    fun checkSimulatedRemoteContainsWithContent(
        fileName: String,
    ...
    fun checkTaskNotRegistered(
        tasksTaskName: String,
        taskName: String,
    ): Unit =
        runBlocking {
            world
                .executeGradle(tasksTaskName)
                .output
                .run(::assertThat)

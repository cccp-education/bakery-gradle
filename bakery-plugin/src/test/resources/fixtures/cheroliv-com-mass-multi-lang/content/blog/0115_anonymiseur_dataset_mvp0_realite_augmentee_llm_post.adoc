...
```
Données brutes client (plans de formation, cours, évaluations)
...
ENTRÉE : Données brutes multi-sources
  ├→ AsciiDoc (plan_formation.adoc) → noms, dates, OF pilote
...
MVP0 → datasets propres accumulés (session après session)
    ↓
Fine-tuning expert métier sur données zéro PII
    ↓
...
CREATE TABLE formation (
  ...
  referentiel CERTIF REFERENCES certification(id),
  organisme OF REFERENCES organisme(id)  -- ← va être anonymisé
);
----

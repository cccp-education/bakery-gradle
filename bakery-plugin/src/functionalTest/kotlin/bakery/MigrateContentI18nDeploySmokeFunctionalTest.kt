...
/**
 * EPIC BKY-I18N-DEPLOY — US DEPLOY-0.
 *
 * Smoke test du dry-run `migrateContentI18n` sur un mini-site JBake natif
 * représentatif de cheroliv.com (3 articles : simple, avec PlantUML, avec
 * source code). Valide que le pipeline parse les 108 articles sans crash,
 * zéro écriture en mode dry-run.
 *
 * Boundary bakery ↔ sites (cf. `.agents/I18N_DEPLOY_BOUNDARY.adoc`) :
...
class MigrateContentI18nDeploySmokeFunctionalTest {
    ...
    fun `dry-run migrateContentI18n parses 3 JBake native articles without writing`() {
        ...
        assertThat(projectDir.resolve("build/i18n-dry-run").exists()).isFalse()
    }
    ...
    fun `dry-run migrateContentI18n logs each parsed adoc file`() {
        ...
        assertThat(result.output).contains("[migrateContentI18n] DRY-RUN — aucun fichier modifié.")
    }

    @Test
    fun `dry-run migrateContentI18n preserves JBake native header format`() {
        createMiniCherolivSite()
    ...
    @Test
    fun `dry-run migrateContentI18n handles article with plantuml block without crash`() {
        createMiniCherolivSite()
    ...
    @Test
    fun `dry-run migrateContentI18n handles article with source code block without crash`() {
        createMiniCherolivSite()

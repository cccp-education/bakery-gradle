...
fun interface ConfigPromptM<out A> {
    ...
    fun <B> flatMap(f: (A) -> ConfigPromptM<B>): ConfigPromptM<B> = ConfigPromptM { env -> f(this.run(env)).run(env) }
    ...
    companion object {
        ...
        fun promptSensitive(propertyName: String): ConfigPromptM<String> =
            ConfigPromptM { env ->

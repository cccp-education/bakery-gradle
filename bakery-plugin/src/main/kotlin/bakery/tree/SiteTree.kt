...
class SiteTree(
    ...
    private fun walkPreOrder(node: SiteNode): List<SiteNode> =
        when (node) {
    ...
    private fun walkPostOrder(node: SiteNode): List<SiteNode> =
        when (node) {
    ...
    fun filter(predicate: (SiteNode) -> Boolean): List<SiteNode> = walk().filter(predicate)

    fun leaves(): List<SiteNode.Article> = walk().filterIsInstance<SiteNode.Article>()

    fun sections(): List<SiteNode> = walk().filter { it.isSection() }

    fun findByPath(path: String): SiteNode? = walk().firstOrNull { it.path == path }
    ...
    fun <T> visit(transform: (SiteNode) -> T): List<T> = walk().map(transform)
}

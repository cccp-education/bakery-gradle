...
object FileSystemManager {
    ...
    internal fun copyFromResourceUrl(
        resourcePath: String,
        targetDir: File,
        project: Project,
    ...
    fun createRepoDir(
        ...
        path.let(::File).apply {
            if (exists() && !isDirectory) {
                if (delete()) {
                    logger.info("$name exists as file and successfully deleted.")
                } else {
                    throw "$name exists and must be a directory".run(::IOException)
                }
            }

            if (exists()) {
                if (deleteRecursively()) {
                    logger.info("$name exists as directory and successfully deleted.")
                } else {
                    throw "$name exists as a directory and cannot be deleted".run(::IOException)
                }
            }
            ...
            if (!exists()) {
                ...
                throw IOException("$name must not exist anymore.")
            }

            if (!exists()) {
                if (mkdirs()) {
                    logger.info("$name as directory successfully created.")
                } else {
                    throw IOException("$name as directory cannot be created.")
                }
            }
        }
        ...
        try {
            bakeDirPath
                ...
                .apply {
                    ...
                    if (!exists()) {
                        logger.info("$name directory successfully deleted.")
                    } else {
                        throw IOException("$name must not exist.")
                    }
                }
            Unit.right()
        } catch (e: Exception) {
    ...
    fun Project.read(configFile: File): Either<Exception, SiteConfiguration> =
        try {
